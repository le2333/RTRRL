...
author: jlemmel
改编自: https://github.com/NicolasZucchet/Online-learning-LR-dependencies
"""
...
def theta_log_init(key, shape, max_phase=6.28):
    """将 theta_log 初始化为 log(max_phase * x),其中 x ~ U[0, 1]。"""
...
class LRUCell(nn.Module):
    ...
    """
    梯度记忆: dh_{t-1}/d lambda, dh_{t-1}/d gamma #1,
              dh_c_{t-1}/d B #2
    """
    ...
    def __call__(self, carry, inputs, resets=None):
        """对给定输入序列计算输出,可选地重置隐藏状态。

        假设输入是形状 (steps, input_dim) 的序列,或形状 (input_dim,) 的单个向量。
        """
        h_tminus1 = carry
        ...
        # 运行 LRU + 输出投影
        # 关于 parallel scan 的细节,见 Smith et al (2022) 的讨论。
...
class OnlineLRUCell(nn.RNNCellBase):
    ...
    def __call__(
        ...
        carry : any
            LRU 单元状态。
        x_t : array
            输入序列。(steps, input_dim) 或 (input_dim,)
        resets : array, optional
            与输入序列等长的布尔数组,默认 None
        force_trace_compute : bool, optional
            即使不需要 autograd 也强制更新梯度迹,默认 False
        ...
        def bwd(residuals, y_t):
            # y_t =(partial{output}/partial{h_{t}},忽略其余
...
class OnlineLRULayer(nn.RNNCellBase):
    ...
    def __init__(
        ...
        """用指定的输出维度、隐藏维度和可塑性类型初始化模型。

        Args:
            d_output (int): 输出层维度。
            d_hidden (int, optional): 隐藏层维度,未提供时默认取 d_output 的值。
            plasticity (str): 使用的可塑性类型,默认 "bptt"。
            kwargs: 传给 LRU 单元的额外参数。
        """
    ...
    def __call__(self, carry, x_t, *args, **kwargs):
        ...
        Args:
            carry (Union[Array, Tuple[Array, ...]]): 上一时间步的 carry 状态。
            x_t (Array): 当前时间步的输入。
            resets (Optional[Array]): 可选的重置信号。
            **args: 传给 LRU 单元的额外参数。

        Returns:
            Tuple[Union[Array, Tuple[Array, ...]], Array]: 更新后的 carry 状态和当前时间步的输出。
        """
