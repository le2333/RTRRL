...
环境包装器现在只给一个 `done`,摔倒与到达步数上限都置 1。算法拿它做两件事,而这两件事需要的不是同一个信号:

- **重置 carry 与资格迹** —— 两种情况都要重置,用 `done` 是对的。
- **自举** —— `δ = r + γ(1 − done)·V(s′) − V(s)` 里那个因子在摔倒时置零是对的:真的终止,后面没有任何一步,`V(s′)=0` 精确表达"失去全部后续存活奖励",这也是为什么不需要额外的失败惩罚。但截断时未来**还在**,只是我们不看了,置零就是错的。策略越好、被截断的比例越高,这个错误信号越频繁。

所以拆成两个:`done = 终止 ∪ 截断` 用于重置,`terminal` 仅指终止,用于自举。算法侧只改一个掩码,`(1 − done)` 换成 `(1 − terminal)`。

前提在包装器:`BraxGymnaxWrapper.step`(`memo/memorax/environments/brax.py:24-34`)返回空 `info`,把 Brax 的 `truncation` 扔了;`AutoResetWrapper` 还会把 `next_obs` 换成新回合的起点,所以截断处真实的 `s′` 也没了。两样都要带出来,否则换成 `terminal` 之后自举的是一个无关状态。其余五个环境包装器不在本轮范围,它们本来就红。

`EpisodeWrapper(env, episode_length=1000)` 写死在同一个文件里,配置文件够不着。它定义任务(同一策略在截断 500 与 1000 下的回报不可比),属于 `environment` 段。

### TD 收语义信号,不收嚼过的数

`td0(*, reward, value, next_value, bootstrap_discount)` 收的是算法先算好的 `γ(1 − done)`。折扣与终止语义都是 TD 自己的定义,却留在了调用方。改为收 `terminal`,由 TD 自己乘。

但 **γ 不归 TD 组件**。它有两个消费者:TD 的自举,和折扣回报迹的尺度估计。两者必须相等——不等就是在按错误的时间跨度缩放奖励——而规格 §4 明确不引入跨参数约束语言,没有机制能强制它们一致。γ 定义的是目标函数的时间跨度,不是任何组件的实现细节,所以它是**算法持有的、不属于任何结构的参数**,传给需要它的组件。

### 环境由算法持有,不搬到调度框架

`drive()` 只拿到 `init_fn`/`train_fn`/`evaluate_fn`,看不见环境;`StreamAC` 自己 `vmap(self.env.step, ...)`。代价是每个算法各接一遍交互,收益是顺序归算法。

搬到 runner 是可行的——两个闭包在构建期拼成一个 scan 体,XLA 仍融合成一个函数,`collect(T) → update` 还能容下 PPO 那类多步更新。但 `T>1` 时行为策略与目标策略"部分新部分旧"是**算法语义**:采样用的是哪一份参数、要不要 off-policy 修正,只有算法知道。把它做成调度参数,就是给出一个选错会静默产生有偏更新的旋钮,而配置面拦不住。

所以维持现状。等三个入口都迁完、交互接线重复了三遍之后,再看那份重复值不值得消除。
...
两者按回合分工:**Aim 存这个回合的统计量,rerun 存这个回合内的指标轨迹。** 想知道 `td_error` 是不是在开头尖峰,去 rerun 看那个回合的序列;想比较两次运行,读 Aim 的标量。两边的单位都是回合,都看不见 chunk。
