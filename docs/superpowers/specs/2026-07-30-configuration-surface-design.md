...
阶段 2 的测试**不得断言任何算法的具体字段**。它测的是加载器:未声明的键被拒绝、越界的覆盖被拒绝、单点 search 只有一个候选、未激活分支塌缩为 placeholder、结构由实验钉死或取 placeholder 而不进搜索、分支参数按 `<结构>.<分支>.<参数>` 成键、缺键即报错。

这些断言用测试内构造的声明,不读任何真实入口。一个断言里出现 `gamma` 或 `kappa` 就说明它测错了层——那是算法的事,不是加载器的事。这样加载器的行为与算法的取值互不牵连,改一个参数的范围不会让 infra 的测试变红。
