"""A verbatim copy of the StreamAC-RTRL kernel as it stood before the rewrite.

Nothing imports this except the test that holds the rewrite to it. It exists
because the rewrite is a change of shape and must not be a change of
arithmetic, and there is no other way to say that with evidence: the goldens
this repository used to carry were deleted, and the run that produced the
comparison curve is not cheap enough to repeat on a guess.

Delete this file, and the test beside it, once the rewrite has been shown
equal. Do not edit it in the meantime -- an edit here quietly turns the test
from a check into a tautology.
"""
