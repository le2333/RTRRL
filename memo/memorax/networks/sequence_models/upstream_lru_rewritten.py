"""The LRU the RTRRL authors rewrote after publishing, for one comparison only.

Source: ``RTRRL-AAAI25`` commit ``4301943``, file ``models/online_lru.py`` -- the
repository's HEAD arithmetic. ``upstream_lru.py`` beside this is the same file at
``b71fd6e``, the revision the paper was published at, and the two exist for the
same reason: ``RewrittenLRUCell`` and ``PublishedLRUCell`` in ``lru_upstream.py``
claim to reproduce a revision each, and a claim about someone else's arithmetic
is worth only the file it is checked against.

That checking found the claim about this revision wrong, which is why this file
was added. Read ``_trace_update`` below and it appears to accumulate: every entry
is ``Lambda * grad_memory[i] + <this step>``. But ``grad_memory`` comes from the
carry, and nothing here ever writes an updated trace back into one. ``f`` is the
primal, and it returns the traces it was handed unless ``force_trace_compute``,
which is a parameter with no caller anywhere in ``rtrrl/`` -- signature,
docstring, and the ``if`` are its only three appearances. ``fwd`` does compute
fresh traces, into the residual, where ``bwd`` spends them on that one step's
gradient and drops them. So every step's ``grad_memory`` is the zero that
``initialize_carry`` made, the ``Lambda *`` term is always zero, and the credit
this revision assigns is one step deep:

    z_lambda = h_{t-1},  z_gamma = x_t B^T,  z_B = outer(exp(gamma_log), x_t)

Which is not what a reader of ``_trace_update`` would say, and not what
``RewrittenLRUCell`` reproduced before this file existed. Comparing against a
transcription rather than against a reading is the difference.

Two further things this revision does, both absent at ``b71fd6e`` and both
reproduced by the arm rather than merely noted. ``LRUCell.__call__``
``reshape(-1, input_dim)`` before its ``associative_scan``, so a single
transition is a length-one sequence, the scan is the identity, and the forward
pass has no recurrence in it at all. And ``OnlineLRULayer`` gained a skip
parameter ``D`` and a ``silu``, so with the recurrence gone that skip is most of
what remains.

The arithmetic is untouched, down to the spelling of ``B_img``, the order of
every multiplication and the commented-out lines in ``rtrl_gradient``. The layout
is upstream's rather than this repository's, so that
``git show 4301943:models/online_lru.py`` diffs against this file in a few lines:
this docstring, the import below, and the removal of the ``__main__``
demonstration. Nothing here may be tidied. A reassociated product is a changed
number, and this file exists to be the number that did not change.
"""
...
from .upstream_seq_util import binary_operator, binary_operator_reset
