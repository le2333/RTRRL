"""StreamAC as upstream Memorax writes it, kept to be checked against.
...
snapshots. It is back because it is the reference, and it is under this name
because the kernel next door took the other one.

The arithmetic here is upstream's, line for line -- the eligibility trace, the
overshooting bound, the objectives, the initialisation. Nothing should import
it to run something; ``stream_ac.StreamAC`` with ``credit="tbptt"`` is meant to
be this algorithm, and the point of keeping the text is to have something that
...
class UpstreamStreamAC:
    ...
    ) -> tuple[UpstreamStreamACState, Array, Array, None]:
        del key
    ...
    def init(self, key: Key) -> UpstreamStreamACState:
        (
            env_key,
            actor_key,
            actor_torso_key,
    ...
    def train(self, key: Key, state: UpstreamStreamACState, num_steps: int):
