"""StreamAC as this repository shipped it before the layering, kept to be
checked against.

Nothing runs this. ``tests/test_layered_parity.py`` drives it beside
``memorax/algorithms/stream_ac.py`` -- which is the same algorithm in four
layers -- and compares every leaf, so a rearrangement that changes a number is
caught without anything having to say what the number should be.
