"""Both CTRNN-RFLO entries, driven through a real Worker child.

The two smokes issue 65 asks for, and the reason it asks for them: everything
else about this algorithm is checked by building the graph in-process and
stepping it, which says nothing about whether an *entry* runs. Between the
graph and a result there is a manifest, a worker, a run document validated
against the deployment contract, a reporter, an S3 upload and -- for the
grouped entry -- a member axis that has to hand each member its own artifacts.
That path has broken after green algorithm tests before, which is why the
acceptance criteria name it separately.
...
from memorax.algorithms.rtrrl_ctrnn_rflo import PARAMETERS
...
def parameters() -> dict:
    """The published settings, at the smallest width that still recurs."""
...
def test_the_worker_runs_the_entry_and_publishes_what_it_declares(
    ...
    """A minimal training run of ``rtrrl_ctrnn_rflo``, end to end.
    ...
    spec = specification(entry="rtrrl_ctrnn_rflo", root=root, aim_path=aim_path, seed=0)
...
def test_the_worker_runs_two_seeds_as_one_group_and_reports_each_alone(
    ...
    """Two seeds through ``rtrrl_ctrnn_rflo_ensemble``, one graph, two results.
    ...
    specs = [
        specification(
            entry="rtrrl_ctrnn_rflo_ensemble", root=root, aim_path=aim_path, seed=seed
        )
        for seed in SEEDS
    ]
