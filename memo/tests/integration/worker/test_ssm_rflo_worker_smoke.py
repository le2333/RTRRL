"""Both dense-SSM-RFLO entries, driven through a real Worker child.
...
why each RFLO entry gets its own smoke rather than inheriting the previous
one's evidence.
...
from memorax.algorithms.rtrrl_ssm_rflo import PARAMETERS
...
def test_the_worker_runs_the_entry_and_publishes_what_it_declares(
    ...
    """A minimal training run of ``rtrrl_ssm_rflo``, end to end.
    ...
    spec = specification(entry="rtrrl_ssm_rflo", root=root, aim_path=aim_path, seed=0)
...
def test_the_worker_runs_two_seeds_as_one_group_and_reports_each_alone(
    ...
    """Two seeds through ``rtrrl_ssm_rflo_ensemble``, one graph, two results.
    ...
    specs = [
        specification(
            entry="rtrrl_ssm_rflo_ensemble", root=root, aim_path=aim_path, seed=seed
        )
        for seed in SEEDS
    ]
