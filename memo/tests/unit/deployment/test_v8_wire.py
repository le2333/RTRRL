"""Every image-side consumer projects the same serialized version-8 run."""
...
FIXTURES = Path(__file__).resolve().parents[4] / "tests" / "contracts" / "v8"
...
def test_one_serialized_run_has_worker_and_entry_projections() -> None:
    ...
    assert worker.contract == entry.contract == CONTRACT_VERSION == 8
...
def test_image_catalog_uses_the_deployment_contract(tmp_path: Path) -> None:
    ...
    assert set(parsed.entries) == {"rtrrl", "stream_ac"}
