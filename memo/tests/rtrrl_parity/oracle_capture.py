...
def _store_tree(
    ...
    for path, leaf in jax.tree_util.tree_leaves_with_path(tree):
        arrays[f"{prefix}/{_path_text(path)}"] = np.asarray(leaf)
    ...
    manifest = {
        ...
        **versions,
        ...
        "state_machine": {
            ...
            "step_key_split": ["next", "action", "dropout"],
            "phase_order": [
                "persisted-action environment step",
                "terminal recurrent reset",
                "current model input",
                "sample next action and value",
                "TD and optimizer update",
                "trace and feedback persistence",
            ],
            "mock_protocol": (
                "one environment, scalar observation, continuous two-vector "
                "action, terminal on transition two, three eager transitions"
            ),
            "normalization": "disabled; pinned running-statistics leaves are None",
        },
