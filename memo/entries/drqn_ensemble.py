"""Deployment composition for DRQN run as one graph over a trial's seeds.

The same algorithm, the same declarations and the same run documents as
``drqn``: what differs is that the worker hands this entry a whole group and
...
main = main_for(
    DRQN, build_request=build_request, runtime_config=runtime_config
)
