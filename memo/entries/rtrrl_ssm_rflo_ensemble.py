"""RTRRL-SSM-RFLO run as one graph over a trial's seeds.

The same algorithm, the same declarations and the same run documents as
``rtrrl_ssm_rflo``: what differs is that the worker hands this entry a whole
...
Everything the recurrence's shape depends on -- the hidden width, the norm ball
``A`` is held inside, whether a normalization follows the cell, which
differentiation credits it -- is declared static, so a group varying one is
refused before anything compiles. What may vary across members is what the
graph reads arithmetically: the discounts, the three trace decays, the two
etas, the entropy rate, and every optimizer's own numbers.
"""
...
from .rtrrl_ssm_rflo import build_request, runtime_config
