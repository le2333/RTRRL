"""Deployment composition for RTRRL over a dense linear state-space torso.

A separate entry from ``rtrrl`` rather than a backbone value it could be given,
even though the recurrence is the LRU's with one constraint lifted -- and that
is exactly why. ``rtrrl``'s state-space backbones are diagonal, and on them the
differentiation family offers ``exact_rtrl`` because RFLO would be a second
name for it. Here ``A`` is dense, the family offers ``rflo``, and the entry
narrows it to that. Two run documents whose only difference was a backbone
value could not have said which of those two things happened. See
``memorax.algorithms.rtrrl_ssm_rflo``.
"""
...
from memorax.algorithms.rtrrl_ssm_rflo import METRICS as METRICS
from memorax.algorithms.rtrrl_ssm_rflo import OBSERVATIONS
from memorax.algorithms.rtrrl_ssm_rflo import PARAMETERS as PARAMETERS
...
def run(reporter, config) -> None:
    algorithm = assemble(RTRRLSsmRflo, build_request(config))
